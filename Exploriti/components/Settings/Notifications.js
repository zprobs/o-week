...
import { GET_NOTIFICATION_SETTINGS, UPDATE_GROUP, UPDATE_USER } from '../../graphql';
...
function Notifications() {
  ...
  const {data, loading, error} = useQuery(GET_NOTIFICATION_SETTINGS, {variables: {id: authState.user.uid}, fetchPolicy: 'cache-and-network'})
  ...
  const onSave = () => {
      if (data) {
        setIsUploading(true)
        const { user } = data
        if (muteMessages !== user.muteMessages || muteEvents !== user.muteEvents || muteGroups !== user.muteGroups) {
  ...
  return (
    <View style={styles.container}>
      <SettingsSwitch title="Mute Messages" setIsEnabled={setMuteMessages} isEnabled={muteMessages} />
      <SettingsSwitch title="Mute Events" setIsEnabled={setMuteEvents} isEnabled={muteEvents} />
      <SettingsSwitch title="Mute Group Notifications" isEnabled={muteGroups} setIsEnabled={setMuteGroups} />
      <SettingsSwitch title="Mute All" setIsEnabled={onMuteAll} isEnabled={muteAll} />
