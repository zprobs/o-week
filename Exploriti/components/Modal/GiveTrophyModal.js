...
const GiveTrophyModal = React.forwardRef(
  /**
   * used by admins to award a group a trophy
   * @param selected {[string]} the array of selected groups to be awarded
   * @param onClose {function}
   * @param ref
   * @returns {JSX.Element}
   */
  ({ selected, onClose }, ref) => {
    ...
    const [getMembers, { data, loading, error: membersError }] = useLazyQuery(
      GET_GROUPS_MEMBERS,
    );
    if (error) {
    ...
    const onDone = async () => {
      ...
      award({ variables: { objects: objects } })
        .then(() => {
          showMessage({
            message:
              'You may have to restart the app to see the trophy on the group screen',
            type: 'info',
            icon: 'auto',
          });
